...
function AdminPanel() {
  ...
  // --- State pour gérer l'affichage Select vs Input ---
  ...
  const submitIntervenant = async (e) => {
    ...
    if (res.ok) {
      ...
      setIsCustomSpec(false); // On remet le select par défaut
      refreshLists();
    } else { setMessage({ texte: "❌ Erreur validation.", type: 'error' }); }
  };

  // --- 3. SOUMISSION SESSION ---
  ...
  const usersIntervenants = listUtilisateurs.filter(u => u.role === 'INTERVENANT');
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.card}>
        ...
        <p style={{fontSize:'0.8rem', color:'#666', marginBottom:'15px'}}>Associez une spécialité à un utilisateur inscrit.</p>
        
        <form onSubmit={submitIntervenant} style={styles.form}>
          
          <label style={styles.label}>Utilisateur (Role Intervenant) :</label>
          ...
          <button type="submit" style={styles.btnGreen}>Valider et Lier</button>
        </form>
      </div>
