...
// Icône Utilisateur SVG (Simple et Pro)
const UserIcon = () => (
...
function ParticipantCatalog() {
  ...
  const handlePayerEtInscrireFinal = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showAlert("Erreur lors de l'enregistrement du paiement en base de données.", "error");
      }
    } catch (error) {
  ...
  // Récupération de l'intervenant depuis la première session (si elle existe)
        ...
        <div style={styles.headerContent}>
          {!selectedFormation ? (
            ...
            <>
              ...
              {/* AJOUT DE LA DUREE DANS LE HEADER */}
                      ...
                      {sessions.map(s => (
                        <div key={s.id} style={styles.sessionItem}>
                          ...
                          <button 
                            ...
                            style={s.nbPlaces > 0 ? styles.btnSmallInscrire : styles.btnSmallComplet}
                            disabled={s.nbPlaces <= 0}
                          >
                            {s.nbPlaces > 0 ? `Réserver` : "Complet"}
                          </button>
                        </div>
                      ))}
                    </div>
            ...
            <aside style={styles.sidebar}>
              <div style={styles.stickyContainer}>
                ...
                <div style={styles.sideCard}>
                  ...
                  <button onClick={() => startPayment(sessions[0]?.id, selectedFormation.prix)} style={styles.btnFullAction}>S'inscrire au cursus</button>
                </div>

                {/* --- NOUVELLE CARTE FORMATEUR EPUREE --- */}
                {intervenantRef && (
                    <div style={styles.sideCard}>
                        {/* Même titre que la carte du dessus pour la cohérence */}
                        ...
                        {/* Statistiques supprimées comme demandé */}
                    </div>
                )}
                {/* ------------------------------------------- */}
...
const styles = {
  ...
  // Modif du sideCard pour permettre l'alignement
  ...
  // Styles pour la carte formateur épurée
